...
const NestedArgBlockSVG: React.FC<Props> = (props) => {
    ...
    return (
        <svg
            viewBox={`0 0 ${(props.blockWidthLines + (ARG_NOTCH_BRIDGE_WIDTH + ARG_NOTCH_WIDTH)) * 10} ${blockLines * 10}`}
            width={`${BlocksConfig.BLOCK_SIZE * (props.blockWidthLines + (ARG_NOTCH_BRIDGE_WIDTH + ARG_NOTCH_WIDTH))}px`}
